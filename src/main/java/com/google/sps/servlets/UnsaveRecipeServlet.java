...
/**
 * Remove recipe from user's list depending on the following parameters:
 * "listName" = "cookbook", "planner". The list to delete from.
 * "isPublic" = Boolean
 * "recipeId" = Long
 *  userToken is the user's Google id_token provided by user-auth.js/getIdToken().
 * "userToken" = String
 */
@WebServlet("/unsave-recipe")
public class UnsaveRecipeServlet extends HttpServlet {
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    if (listName.equals(COOKBOOK)) {
      ...
      currentUser.removePlannerKey(recipeKey);
    } else {
      System.out.println("Invalid listName " + destination + ". No action.");
    }
  }
}
